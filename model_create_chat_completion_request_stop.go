...
func (dst *CreateChatCompletionRequestStop) UnmarshalJSON(data []byte) error {
	...
	if match > 1 { // more than 1 match
		...
		return fmt.Errorf("data matches more than one schema in oneOf(CreateChatCompletionRequestStop)")
	} else if match == 1 {
		return nil // exactly one match
	} else { // no match
		return fmt.Errorf("data failed to match schemas in oneOf(CreateChatCompletionRequestStop)")
	}
}
...
func NewNullableCreateChatCompletionRequestStop(val *CreateChatCompletionRequestStop) *NullableCreateChatCompletionRequestStop {
	return &NullableCreateChatCompletionRequestStop{value: val, isSet: true}
}
